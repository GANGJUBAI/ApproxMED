...
ResultSoure="/path/to/src/result/"
...
do
       echo $err
       echo " ********************  $i is being processed  with QSORT ***********************************"

       Path=$DataSource"Miranda/"$i".d64"
       Result=$ResultSoure"3D/Time_MirandaQsort.result"
       ./Main_qsort $Path 256 384 384 D 3 >> $Result
...
do
       ...
       Result=$ResultSoure"3D/Time_HurricaneQsort.result"
       ./Main_qsort $Path 500 500 100 F 3 >> $Result
...
do
       ...
       Result=$ResultSoure"3D/Time_NYXQsort.result"
       ./Main_qsort $Path 512 512 512 F 3 >> $Result
...
do 
       ...
       Result=$ResultSoure"2D/Time_powerQsort.result"

       echo "power is processed with para (24 60 723)" >> $Result
       ./Main_qsort $Path 24 60 723 F 2 >> $Result

       echo "power is processed with para (3 60 5784)" >> $Result
       ./Main_qsort $Path 3 60 5784 F 2 >> $Result

       echo "power is processed with para (12 60 1446)" >> $Result
       ./Main_qsort $Path 12 60 1446 F 2 >> $Result
...
do
    ...
    echo " ******************** $FileName is being processed with QSORT  ***********************************"
    
    Path=$DataSource2"MinstMoving/"$FileName".bin"
    Result=$ResultSoure"2D/Time_MinstMovingQsort.result"

    j=1
    while [ $j -le 1000 ]; 
    do
        echo " ***************************** $FileName (loop=$j) is being processed with QSORT  **************************"
        ./Main_qsort $Path 64 64 20 I 2 $j 2 >> $Result
        j=`expr $j \* 10`  
...
do 
    ...
    Path=$DataSource2$i".bin"
    Result=$ResultSoure"2D/Time_lenaQsort.result"
    
    j=1
    while [ $j -le 1000 ]; 
    do
        echo " ***************************** $i (loop=$j) is being processed with QSORT  **************************"
        ./Main_qsort $Path 512 512 $j I 2 >> $Result
        j=`expr $j \* 10`
    done
    echo "**************************** process $i  is done ********************************"
...
do
    ...
    for j 
    do
        echo "**************************** Zipf_$i"_"$j is being processed with QSORT ***********************************"

        Path=$DataSource2"Zipf_"$j"_[1,10]_"$count".bin"
        Result=$ResultSoure"1D/Time_ZipfQsort.result"
        ...
        while [ $k -le 100 ]; 
        do
            echo "**************************** Zipf_$i_$j(loop=$k) is being processed with QSORT ***********************************"
            ./Main_qsort $Path $k $i I 1 >> $Result
            k=`expr $k \* 10`
        done    
...
do
    echo $err
    count=$(echo $i | grep -o 0 | wc -l)
    echo "**************************** normal_10$count is being processed with QSORT ***********************************"

    Path=$DataSource2"normal_10"$count".bin"
    Result=$ResultSoure"1D/Time_normalQsort.result"
    ...
    while [ $j -le 100 ]; 
    do
        echo "**************************** normal_10$count(loop=$j) is being processed with QSORT ***********************************"
        ./Main_qsort $Path $j $i I 1 >>$Result
        j=`expr $j \* 10`
    done 
    i=`expr $i \* 10`
...
do
    echo $err
    count=$(echo $i | grep -o 0 | wc -l)
    echo "**************************** constant_10$count is being processed with QSORT ***********************************"

    Path=$DataSource2"constant_10"$count".bin"
    Result=$ResultSoure"1D/Time_constantQsort.result"
    ...
    while [ $j -le 100 ]; 
    do
        echo "**************************** constant_10$count(loop=$j) is being processed with QSORT ***********************************"
        ./Main_qsort $Path $j $i I 1 >>$Result
        j=`expr $j \* 10`
    done 
    i=`expr $i \* 10`
