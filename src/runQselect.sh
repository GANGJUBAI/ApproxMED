...
do
       echo $err
       echo " ********************  $i is being processed  with QSELECT ***********************************"

       Path=$DataSource"Miranda/"$i".d64"
       Result=$ResultSoure"3D/Time_MirandaQSelect.result"
       ./Main_qSelect $Path 256 384 384 D 3 >> $Result
...
do
       ...
       Result=$ResultSoure"3D/Time_HurricaneQSelect.result"
       ./Main_qSelect $Path 500 500 100 F 3 >> $Result
...
do
       ...
       Result=$ResultSoure"3D/Time_NYXQSelect.result"
       ./Main_qSelect $Path 512 512 512 F 3 >> $Result
...
do 
       ...
       Result=$ResultSoure"2D/Time_powerQSelect.result"

       echo "power is processed with para (24 60 723)" >> $Result
       ./Main_qSelect $Path 24 60 723 F 2 >> $Result

       echo "power is processed with para (3 60 5784)" >> $Result
       ./Main_qSelect $Path 3 60 5784 F 2 >> $Result

       echo "power is processed with para (12 60 1446)" >> $Result
       ./Main_qSelect $Path 12 60 1446 F 2 >> $Result
...
do
    ...
    echo " ******************** $FileName is being processed with QSelect  ***********************************"
    
    Path=$DataSource2"MinstMoving/"$FileName".bin"
    Result=$ResultSoure"2D/Time_MinstMovingQSelect.result"

    j=1
    while [ $j -le 1000 ]; 
    do
        echo " ***************************** $FileName (loop=$j) is being processed with QSelect  **************************"
        ./Main_qSelect $Path 64 64 20 I 2 $j 2 >> $Result
        j=`expr $j \* 10`  
...
do 
    ...
    Path=$DataSource2$i".bin"
    Result=$ResultSoure"2D/Time_lenaQSelect.result"
    
    j=1
    while [ $j -le 1000 ]; 
    do
        echo " ***************************** $i (loop=$j) is being processed with QSelect  **************************"
        ./Main_qSelect $Path 512 512 $j I 2 >> $Result
        j=`expr $j \* 10`
    done
    echo "**************************** process $i  is done ********************************"
...
do
    ...
    for j 
    do
        echo "**************************** Zipf_$i"_"$j is being processed with QSelect ***********************************"

        Path=$DataSource2"Zipf_"$j"_[1,10]_"$count".bin"
        Result=$ResultSoure"1D/Time_ZipfQSelect.result"
        ...
        while [ $k -le 100 ]; 
        do
            echo "**************************** Zipf_$i_$j(loop=$k) is being processed with QSelect ***********************************"
            ./Main_qSelect $Path $k $i I 1 >> $Result
            k=`expr $k \* 10`
        done    
...
do
    echo $err
    count=$(echo $i | grep -o 0 | wc -l)
    echo "**************************** normal_10$count is being processed with QSelect ***********************************"

    Path=$DataSource2"normal_10"$count".bin"
    Result=$ResultSoure"1D/Time_normalQSelect.result"
    ...
    while [ $j -le 100 ]; 
    do
        echo "**************************** normal_10$count(loop=$j) is being processed with QSelect ***********************************"
        ./Main_qSelect $Path $j $i I 1 >>$Result
        j=`expr $j \* 10`
    done 
    i=`expr $i \* 10`
...
do
    echo $err
    count=$(echo $i | grep -o 0 | wc -l)
    echo "**************************** constant_10$count is being processed with QSelect ***********************************"

    Path=$DataSource2"constant_10"$count".bin"
    Result=$ResultSoure"1D/Time_constantQSelect.result"
    ...
    while [ $j -le 100 ]; 
    do
        echo "**************************** constant_10$count(loop=$j) is being processed with QSelect ***********************************"
        ./Main_qSelect $Path $j $i I 1 >>$Result
        j=`expr $j \* 10`
    done 
    i=`expr $i \* 10`
