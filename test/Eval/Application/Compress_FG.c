...
#include "fpc.h"
#include "SPC.h"
#include "/home/lff/ApproxMD/src/ApproxMD_FG.h"
#include <sys/time.h>
...
int main(int argc, char *argv[])
{
    ...
    size_t result = fread(data, dataLength, sizeof(double), ori_fp);
    fclose(ori_fp);
    ...
    for (i = 0; i < rows; i++)
        for (j = 0; j < cols; j++)
        {
            if (j == 0)
            {
                ...
                {
                case 0:
                    mediantemp = medianFindStage1(double)(delta_X, 47, layers, &mean, &sigma);
                    findMedianStage = 1;
                    break;
                case 1:
                    mediantemp = medianFindStage2(double)(delta_X, 47, layers, &mean, &sigma);
                    break;
                }
    ...
    printf("fpc+/fpzip+ FG-MED median time is %fms\n", endtime);
    ...
    printf("fpc+/fpzip+ with FG-MED precondition time is %fms\n", endtime);
    ...
    gettimeofday(&end, NULL);
    endtime = (double)1000 * (end.tv_sec - start.tv_sec) + ((end.tv_usec - start.tv_usec) / 1000.0);
    printf("fpc ori+ with FG-MED compress time is %fms\n", endtime);
    fclose(fp);
    fp = NULL;
    printf("fpc ori+ with FG-MED compression ratio=%lf\n", (double)dataLength * sizeof(double) / c_size_fpc);

    gettimeofday(&start, NULL);
    c_size_fpzip = fpzip_double_array(data, dataLength, 1, 1, 64);
    gettimeofday(&end, NULL);
    endtime = (double)1000 * (end.tv_sec - start.tv_sec) + ((end.tv_usec - start.tv_usec) / 1000.0);
    printf("fpzip ori+ with FG-MED compress time is %fms\n", endtime);
    printf("fpzip ori+ with FG-MED compression ratio=%lf\n", (double)dataLength * sizeof(double) / c_size_fpzip);
