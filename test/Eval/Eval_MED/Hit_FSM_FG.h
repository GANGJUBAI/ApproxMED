...
#define _getAMD1D_IMPL(T)                                                                                       \
    void getAMD1D(T)(int rows, T *data, T *Level_X, int Level_X_size, int groupNum)                             \
    {                                                                                                           \
        struct timeval start, end;                                                                              \
        double mean = 0, median = 0, sigma = 0, Emedian = 0;                                                    \
        int findMedianStage = 0;                                                                                \
        double relativeErr = 0;                                                                                 \
        double absErr = 0;                                                                                      \
        int HALF_LENGTH = Level_X_size / 2;                                                                     \
                                                                                                                \
        ...
        {                                                                                                       \
            ...
            {                                                                                                   \
            case 0:                                                                                             \
                median = medianFindStage1(T)(Level_X, groupNum, Level_X_size, &mean, &sigma);                   \
                findMedianStage = 1;                                                                            \
                break;                                                                                          \
            case 1:                                                                                             \
                median = medianFindStage2(T)(Level_X, groupNum, Level_X_size, &mean, &sigma);                   \
                break;                                                                                          \
            }                                                                                                   \
        }                                                                                                       \
...
#define _process_stream1D_IMPL(T)                                           \
    int process_stream1D(T)(int rows, int cols, FILE *ori_fp, int groupNum) \
    {                                                                       \
        ...
        getAMD1D(T)(rows, data, Level_X, Level_X_size, groupNum);           \
                                                                            \
        free(data);                                                         \
        data = NULL;                                                        \
        free(Level_X);                                                      \
...
#define _getAMD2D_IMPL(T)                                                                                       \
    void getAMD2D(T)(int rows, int cols, int layers, T *data, T *Level_X, int Level_X_size, int groupNum)       \
    {                                                                                                           \
        struct timeval start, end;                                                                              \
        double mean = 0, median = 0, sigma = 0, Emedian = 0;                                                    \
        int findMedianStage = 0;                                                                                \
...
#define _process_stream2D_IMPL(T)                                                       \
    int process_stream2D(T)(int rows, int cols, int layers, FILE *ori_fp, int groupNum) \
    {                                                                                   \
        ...
        T *Level_X = (T *)malloc(Level_X_size * sizeof(T));                             \
                                                                                        \
        getAMD2D(T)(rows, cols, layers, data, Level_X, Level_X_size, groupNum);         \
                                                                                        \
        free(data);                                                                     \
        data = NULL;                                                                    \
        free(Level_X);                                                                  \
...
#define _process_stream2Ds3_IMPL(T)                                                                  \
    int process_stream2Ds3(T)(int rows, int cols, int layers, int loops, FILE *ori_fp, int groupNum) \
    {                                                                                                \
...
#define _process_stream2D3_IMPL(T)                                                       \
    int process_stream2D3(T)(int rows, int cols, int layers, FILE *ori_fp, int groupNum) \
    {                                                                                    \
        int dataLength = rows * cols * layers;                                           \
        int Level_X_size = rows * cols;                                                  \
                                                                                         \
...
#define _getAMD3D_IMPL(T)                                                                                       \
    void getAMD3D(T)(int rows, int cols, int layers, T *data, T *Level_X, int Level_X_size, int groupNum)       \
    {                                                                                                           \
        struct timeval start, end;                                                                              \
        double mean = 0, median = 0, sigma = 0, Emedian = 0;                                                    \
        int findMedianStage = 0;                                                                                \
...
#define _process_stream3D2_IMPL(T)                                                       \
    int process_stream3D2(T)(int rows, int cols, int layers, FILE *ori_fp, int groupNum) \
    {                                                                                    \
        int dataLength = rows * cols * layers;                                           \
        int Level_X_size = cols;                                                         \
        ...
        T *Level_X = (T *)malloc(Level_X_size * sizeof(T));                              \
                                                                                         \
        getAMD3D(T)(rows, cols, layers, data, Level_X, Level_X_size, groupNum);          \
                                                                                         \
        free(data);                                                                      \
        data = NULL;                                                                     \
        free(Level_X);                                                                   \
...
#define _process_stream3D3_IMPL(T)                                                                  \
    int process_stream3D3(T)(int rows, int cols, int layers, int loops, FILE *ori_fp, int groupNum) \
    {                                                                                               \
        int dataSize = rows * cols * layers;                                                        \
        int dataLength = dataSize * loops;                                                          \
        int Level_X_size = cols;                                                                    \
...
#define _process_stream3D_IMPL(T)                                                       \
    int process_stream3D(T)(int rows, int cols, int layers, FILE *ori_fp, int groupNum) \
    {                                                                                   \
        int dataLength = rows * cols * layers;                                          \
        int Level_X_size = cols;                                                        \
                                                                                        \
