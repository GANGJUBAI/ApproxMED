...
#define _max_min(T) max_min$_##T##_$
#define max_min(T) _max_min(T)

#define _max_min_IMPL(T)                                      \
    void max_min(T)(int dataLength, T *array, T *max, T *min) \
    {                                                         \
        T Max = *max;                                         \
        T Min = *min;                                         \
        Max = array[0];                                       \
...
#define max_min_IMPL(T) _max_min_IMPL(T)
max_min_IMPL(int);
max_min_IMPL(float);
max_min_IMPL(double);
    ...
    {                                                                                                           \
        ...
        {                                                                                                       \
            ...
            {                                                                                                   \
                ...
                /*printf("Level_X[%d] is %.10f\n",j,(double)Level_X[j]);*/                                      \
            }                                                                                                   \
            switch (findMedianStage)                                                                            \
            {                                                                                                   \
            case 0:                                                                                             \
                median = medianFindStage1(T)(Level_X, 8, Level_X_size, &mean, &sigma);                          \
                findMedianStage = 1;                                                                            \
                break;                                                                                          \
            case 1:                                                                                             \
                median = medianFindStage2(T)(Level_X, 8, Level_X_size, &mean, &sigma);                          \
                break;                                                                                          \
            }                                                                                                   \
            qsort(Level_X, Level_X_size, sizeof(T), cmpfunc(T));                                                \
            Emedian = Level_X[HALF_LENGTH];                                                                     \
            max_min(T)(Level_X_size, Level_X, &max, &min);                                                      \
            if (-Eps < (max - min) & (max - min)< Eps)                                                          \
            {                                                                                                   \
                absErr = fabs(median - Emedian);                                                                \
                printf("absErr is %.15f\n", absErr);                                                            \
            }                                                                                                   \
            else                                                                                                \
            {                                                                                                   \
                relativeErr = fabs(median - Emedian) / (max - min);                                             \
                printf("relativeErr is %.15f\n", relativeErr);                                                  \
            }                                                                                                   \
        }                                                                                                       \
    ...
    {                                                                                                           \
        ...
        {                                                                                                       \
            ...
            {                                                                                                   \
                ...
                qsort(Level_X, Level_X_size, sizeof(T), cmpfunc(T));                                            \
                Emedian = Level_X[HALF_LENGTH];                                                                 \
                /*printf("Exact median is %.15f\n",Emedian); */                                                 \
                /*printf("approx median is %.15f\n",median);\*/                                                 \
                /*if (-Eps < Emedian && Emedian < Eps)                                                          \
                {                                                                                               \
                    absErr = fabs(median - Emedian);                                                            \
                    printf("absErr is %.15f\n", absErr);                                                        \
                }                                                                                               \
                ...
                {                                                                                               \
                    relativeErr = fabs(median - Emedian) / Emedian;                                             \
                    printf("relativeErr is %.15f\n", relativeErr);                                              \
                } */                                                                                            \
                max_min(T)(Level_X_size, Level_X, &max, &min);                                                  \
                if (-Eps < (max - min) & (max - min)< Eps)                                                      \
...
#define _process_stream3D2_IMPL(T)                                         \
    ...
    {                                                                      \
        ...
        size_t result = fread(data, row_size, sizeof(T), ori_fp);          \
        fclose(ori_fp);                                                    \
        ori_fp = NULL;                                                     \
        for (int i = 1; i < layers; i++)                                   \
        {                                                                  \
